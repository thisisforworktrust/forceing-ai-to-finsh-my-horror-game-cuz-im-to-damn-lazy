...
func _on_noiseplay_body_entered(body: Node) -> void:
	if body != self:
		return
	if TaskManager.game_state != "playing":
		return
	if TaskManager.is_done("inspect_crash"):
		return

	TaskManager.complete("inspect_crash")
	TaskManager.set_objective("Run to the front door")
